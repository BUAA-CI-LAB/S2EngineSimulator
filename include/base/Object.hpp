    ...
    public:
        ...
        inline void ConnectTo(const Port& port){
            if (!(!port.isInput
               ...
               &&(this->countPtr == nullptr)
               &&(this->bitWidth == port.bitWidth))){
                std::cout<<  !port.isInput            <<std::endl
                         <<(  port.dataPtr != nullptr)<<std::endl
                         << this->isInput             <<std::endl
                         <<(this-> dataPtr == nullptr)<<std::endl
                         <<(this->countPtr == nullptr)<<std::endl
                         <<(this->bitWidth == port.bitWidth)<<std::endl;
                printf("%d\n",this->bitWidth);
                printf("%d\n",port.bitWidth);
                getchar();
            }
        ...
        inline void PrintAddr() const{
            std::cout<<"dataPtr:"<<this->dataPtr<<" countPtr:"<<this->countPtr;
            return;
        }
        inline PortDataType Data() const{
            assert(this->isInput);
            return ( (*(this->dataPtr)) & (Object::mask   [this->bitWidth])    );
        }
        inline bool IsNeg() const{
            assert(this->isInput);
            return (((*(this->dataPtr)) &  Object::highest[this->bitWidth] )!=0);
        }

#ifndef __COMPONENT_MIXED_CE_HPP
#define __COMPONENT_MIXED_CE_HPP

#include "../base/Object.hpp"
#include "ReMixedPE.hpp"

#include "../resource/BaseSequLogic.hpp"

class MixedCE: public ClockedObject{
public:
    enum class CE_CTRL{
    ...
    static const enum MixedCE::CE_CTRL InsTable[];
    ...
    static inline PortDataType CompressXEO(
        ...
        return (ReMixedPE::CompressXEO(x,eox,offset,width)<<1) + 1;
    }
    ...
    inline static enum CE_CTRL Decode(PortDataType portData){
        assert((portData>>1) < MixedCE::CtrlNum
            &&((portData& 1) == 0));
        return MixedCE::InsTable[portData>>1];
    }
...
private:
    ...
    inline void InitializePorts() override{
        this->ports.clear();
        this->ports.reserve(MixedCE::PortName::PortNum());
        /** ---------- up -------- **/
        ...
        +ReMixedPE::OFFSET_BIT_WIDTH,PortDirection::OUT));/** BufOutUp   2+X_BIT+XL_BIT out  **/
        ...
        +ReMixedPE::     X_BIT_WIDTH
        +ReMixedPE::    XE_BIT_WIDTH
        +ReMixedPE::OFFSET_BIT_WIDTH,PortDirection::IN ));/** BufInDown  2+X_BIT+XL_BIT in   **/
        ...
        +ReMixedPE::     X_BIT_WIDTH
        +ReMixedPE::    XE_BIT_WIDTH
        +ReMixedPE::OFFSET_BIT_WIDTH,PortDirection::IN ));/** XELIn    2+X_BIT+XL_BIT  in  **/
    ...
    inline bool LeftIsEOX(){
        if (ReMixedPE::DecompressXEO_Mark(this->ports[PORT_INDEX(XELIn)].Data()>>1)==0)
            return (ReMixedPE::DecompressXEO_EOX(this->ports[PORT_INDEX(XELIn)].Data()>>1)==1);
        if (this->hasLeftEOX){
            assert((ReMixedPE::DecompressXEO_EOX(this->ports[PORT_INDEX(XELIn)].Data()>>1)==1));
            this->hasLeftEOX = false;
            return true;
        }
        this->hasLeftEOX = (ReMixedPE::DecompressXEO_EOX(this->ports[PORT_INDEX(XELIn)].Data()>>1)==1);
        return false;
    }
    inline bool DownIsEOX(){
        if (ReMixedPE::DecompressXEO_Mark(this->ports[PORT_INDEX(BufInDown)].Data())==0)
            return (ReMixedPE::DecompressXEO_EOX(this->ports[PORT_INDEX(BufInDown)].Data())==1);
        if (this->hasDownEOX){
            assert((ReMixedPE::DecompressXEO_EOX(this->ports[PORT_INDEX(BufInDown)].Data())==1));
            this->hasDownEOX = false;
            return true;
        }
        this->hasDownEOX = (ReMixedPE::DecompressXEO_EOX(this->ports[PORT_INDEX(BufInDown)].Data())==1);
        return false;
    }
    ...
    inline void  DownBufPop (){
        this->downFIFOPop = 1;
        this->ports[MixedCE::BufInDown.ToIndex()].Read();
        return;
    }
    ...
    inline void RightBufPushZero(){
        if(!this->hasBZ){
            this->hasBZ = true;
            this->xexlFIFOOut  = ReMixedPE::CompressXEO(0,1,1,0);
            this->xexlFIFOPush = 1;
        }
        else{
            this->xexlFIFOOut  = 0;
    ...
    inline void RightBufPushAsyn(){
        this->xexlFIFOOut  = this->ports[PORT_INDEX(XELIn)].Data()>>1;
        this->xexlFIFOPush = 1;
        return;
    }
    ...
    inline void    UpBufPushAsyn(){
        this->bufFIFOIn   = this->ports[PORT_INDEX(XELIn)].Data()>>1;
        this->bufFIFOPush = 1;
        return;
    }
...
public:
    ...
    /** ---------- up -------- **/
    static const MixedCE::PortName BufPopUp;     /**          1-bit  in  **/
    static const MixedCE::PortName BufOutUp;     /** 2+X_BIT+XL_BIT  out **/
    static const MixedCE::PortName BufEmptyUp;   /**          1-bit  out **/
    /** --------- down ------- **/
    static const MixedCE::PortName BufPopDown;   /**          1-bit  out **/
    static const MixedCE::PortName BufInDown;    /** 2+X_BIT+XL_BIT  in  **/
    static const MixedCE::PortName BufEmptyDown; /**          1-bit  in  **/
    /** --------- left ------- **/
    static const MixedCE::PortName XELIn;        /** 2+X_BIT+XL_BIT  in  **/
    static const MixedCE::PortName XELPop;       /**          1-bit  out **/
    static const MixedCE::PortName XELEmpty;     /**          1-bit  in  **/
    /** -------- right ------- **/
    static const MixedCE::PortName OOut;         /** 2+X_BIT+XL_BIT  out **/
    static const MixedCE::PortName OPush;        /**          1-bit  out **/
    static const MixedCE::PortName OFull;        /**          1-bit  in  **/

    MixedCE( PortDataType  downFIFOPop  = 0,
        ...
        bool hasLeftEOX    = false,
        bool hasDownEOX    = false,
        enum CE_CTRL state = CE_CTRL::NONE
       ):bufFIFO       (
         ReMixedPE::  MARK_BIT_WIDTH
       + ReMixedPE::     X_BIT_WIDTH
       + ReMixedPE::    XE_BIT_WIDTH
       + ReMixedPE::OFFSET_BIT_WIDTH,MixedCE::BUF_FIFO_LEN,
       ...
       + ReMixedPE::     X_BIT_WIDTH
       + ReMixedPE::    XE_BIT_WIDTH
       + ReMixedPE::OFFSET_BIT_WIDTH,PortDirection::OUT),
         pBufFIFOPush  (1,PortDirection::OUT)
    {
        ...
        this->hasLeftEOX   = hasLeftEOX;
        this->hasDownEOX   = hasDownEOX;
        this->InitializePorts();
        this->OutputConnect  ();
        return;
    }
    MixedCE(const MixedCE& ce)
        :bufFIFO       (
         ReMixedPE::     X_BIT_WIDTH
        +ReMixedPE::    XE_BIT_WIDTH
        +ReMixedPE::OFFSET_BIT_WIDTH,MixedCE::BUF_FIFO_LEN,
         ...
         pBufFIFOPush  (1,PortDirection::OUT)
    {
        ...
        this->state        = ce.state;
        this->hasLeftEOX   = ce.hasLeftEOX;
        this->hasDownEOX   = ce.hasDownEOX;
        this->InitializePorts();
        this->OutputConnect  ();
        return;
    }
    ...
    inline void InnerConnect() override{
        ...
        this->bufFIFO.Clone(
            FIFO::Pop,
            this->ports[MixedCE::BufPopUp.ToIndex()]);
    ...
    inline bool Run(){
        ...
        case CE_CTRL::NONE : /// idle & initialize state
            ...
            if (this->LeftBufEmpty()){
                ...
                this->state = MixedCE::Decode(this->LeftBufIn());
                this-> LeftBufPop ();
                this-> DownBufNotPop ();
                this->RightBufNotPush();
                this->   UpBufNotPush();

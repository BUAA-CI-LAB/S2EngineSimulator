...
public:
    ...
    public:
        ...
        /** the relevant port of weight and output-mark **/
            ///------------- upper side -------------///
            FupIn     =  0, /** 1-bit in    reg UpFull **/
            FupClr    =  1, /** 1-bit out   reg UpFull **/
            ///------------- under side -------------///
            FdownIn   =  2, /** 1-bit in     input-port FdownIn  **/
            FdownSet  =  3, /** 1-bit out   output-port FdownSet **/
            OWoutEn   =  4, /** 1-bit out   output-port OWoutEn  **/
        /** the relevant port of feature **/
            ///------------- left  side -------------///
            FleftIn   =  5, /** 1-bit in    reg LeftFull **/
            FleftClr  =  6, /** 1-bit out   reg LeftFull **/
            ///------------- right side -------------///
            FrightIn  =  7, /** 1-bit in     input-port FrightIn  **/
            FrightSet =  8, /** 1-bit out   output-port FrightSet **/
            XoutEn    =  9, /** 1-bit out   output-port XoutEn    **/
        /** the ports connected to CL1 **/
            CalcDone  = 10, /** 1-bit in    combinational logical CL1 **/
        /** the ports connected to counter **/
            CountEn   = 11, /** 1-bit out   counter **/
            CountRst  = 12, /** 1-bit out   counter **/
        /** the ports connected to MAC buffer **/
            OWEn      = 13, /** 1-bit out   reg   weight-buffer **/
            XEn       = 14, /** 1-bit out   reg  feature-buffer **/
          /*************/
            PortNumber
        };
    private:
...
private:
    PortDataType
        ...
        sDDownOut,     /** 24-bit out   output-port    Sum  **/
        sDDownEn,      /**  1-bit out   output-port    Sum  **/
        eofClr,        /**  1-bit out   markReg EndOfF **/
        sDDEOFSet;     /**  1-bit out   output-port SddEOFSet  **/

    enum class TransType:uint8_t{
        UNRECEIVED,
        /** has not received value from up-of-up cell
         *  can trans to state RECEIVED implicitly
         **/
          RECEIVED,
        /** has received value from up-of-up cell
         *  can only trans from state UNRECEIVED implicitly
         *
         **/
         FORWARDED,
            TOSENT
    ...
    inline bool IsEndOfForward(){
        return this->ports[PORT_INDEX(EndOfFow)].Data()==1;
    }
    inline bool ReceivedForwarded(){
        return this->ports[PORT_INDEX(FupupIn)].Data()==1;
    }
    inline bool DDownCanForward(){
        return this->ports[PORT_INDEX(FddownIn)].Data()==0;
    }
    inline bool SummationToSend(){
        return ((this->ports[PORT_INDEX(   Obuf  )].Data()==1)
             &&  this->ports[PORT_INDEX(CountDone)].Data()==1);
    }
    inline bool InnerCalcDone(){
        return this->ports[PORT_INDEX(   Obuf  )].Data()==0
            && this->ports[PORT_INDEX(CountDone)].Data()==1;
    }
    inline bool CalcDone(){
        return this->ports[PORT_INDEX(CountDone)].Data()==1;
    }
    inline PortDataType MA(){
        return  this->ports[PORT_INDEX(Wbuf     )].Data()
              * this->ports[PORT_INDEX(Xbuf     )].Data()
              + this->ports[PORT_INDEX(InnerSreg)].Data();
    }
    inline PortDataType M(){
        return  this->ports[PORT_INDEX(Wbuf)].Data()
              * this->ports[PORT_INDEX(Xbuf)].Data();
    }
    inline void ForwardUpUp(){
        this->fUpUpClr  = 1;
        this->eofClr    = 1;
        this->fDDownSet = 1;
        this->sDDownOut = this->ports[PORT_INDEX(Sin)].Data();
        this->sDDownEn  = 1;
        this->sDDEOFSet = 0;
        return;
    }
    inline void NotForward(){
        ...
        this->eofClr    = 0;
        this->fDDownSet = 0;
        this->sDDownOut = this->MA();
        this->sDDownEn  = 0;
        this->sDDEOFSet = 0;
        return;
    }
    inline void ForwardMA(){
        this->fUpUpClr  = 0;
        this->eofClr    = 0;
        this->fDDownSet = 1;
        this->sDDownOut = this->MA();
    ...
    inline void NotWriteBack(){
        this->innerSEn  = 0;
        this->innerSOut = this->MA();
        this->calcDone  = 0;
        return;
    }
    inline void WriteBackMA(){
        this->innerSEn  = 1;
        this->innerSOut = this->MA();
        this->calcDone  = this->InnerCalcDoneinnerCalcDone;
        return;
    }
    inline void WriteBackM(){
        this->innerSEn  = 1;
        this->innerSOut = this->M();
        this->calcDone  = this->InnerCalcDone();
        return;
    }
    ...
    inline void InitializePorts() override{
        ...
        this->ports.push_back(Port(24,PortDirection::IN )); /** Sin      24-bit in  reg Sum **/
        this->ports.push_back(Port( 1,PortDirection::IN )); /** EndOfFow  1-bit in  markReg EndOfF **/
        this->ports.push_back(Port( 1,PortDirection::OUT)); /** EOFClr    1-bit out markReg EndOfF **/
            ///------------ down of down ------------///
        this->ports.push_back(Port( 1,PortDirection::IN )); /** FddownIn   1-bit in   input-port FddownIn  **/
        this->ports.push_back(Port( 1,PortDirection::OUT)); /** FddownSet  1-bit out output-port FddownSet **/
        this->ports.push_back(Port(24,PortDirection::OUT)); /** SddownOut 24-bit out output-port SddownOut **/
        this->ports.push_back(Port( 1,PortDirection::OUT)); /** SddownEn   1-bit out output-port SddownEn  **/
        this->ports.push_back(Port( 1,PortDirection::OUT)); /** SddEOFSet  1-bit out output-port SddEOFSet **/
            ///------------ partial  reg ------------///
    ...
    inline void OutputConnect() override{
        ...
        this->ports[PORT_INDEX(EOFClr   )].Clone(this->eofClr   );
        ...
        this->ports[PORT_INDEX(SddownOut)].Clone(this->sDDownOut);
        this->ports[PORT_INDEX(SddownEn )].Clone(this->sDDownEn );
        this->ports[PORT_INDEX(SddEOFSet)].Clone(this->sDDEOFSet);
        assert(this->OutputConnected());
        return;
    }
...
public:
    ...
    public:
        ...
        /** the relevant port of partial sum **/
            ///------------   up of up   ------------///
            FupupIn   =  5, /**  1-bit in    markReg UpUpFull **/
            FupupClr  =  6, /**  1-bit out   markReg UpUpFull **/
            Sin       =  7, /** 24-bit in    reg Sum **/
            EndOfFow  =  8, /**  1-bit in    markReg EndOfF **/
            EOFClr    =  9, /**  1-bit out   markReg EndOfF **/
            ///------------ down of down ------------///
            FddownIn  = 10, /**  1-bit in     input-port FddownIn  **/
            FddownSet = 11, /**  1-bit out   output-port FddownSet **/
            SddownOut = 12, /** 24-bit out   output-port SddownOut **/
            SddownEn  = 13, /**  1-bit out   output-port SddownEn  **/
            SddEOFSet = 14, /**  1-bit out   output-port SddEOFSet **/
            ///------------ partial reg ------------///
            InnerSreg = 15, /** 24-bit in    reg ParSum **/
            InnerSout = 16, /** 24-bit out   reg ParSum **/
            InnerSen  = 17, /**  1-bit out   reg ParSum **/
            /*************/
            PortNumber
        };
    private:
    ...
    PE_CL1(
        ...
        PortDataType sDDownOut  =0, /**  1-bit out   output-port SddownOut  **/
        PortDataType sDDownEn   =0, /**  1-bit out   output-port SddownEn   **/
        PortDataType sDDEOFSet  =0, /**  1-bit out   output-port SddEOFSet  **/
        PortDataType eofClr     =0, /**  1-bit out   markReg EndOfF **/
        PortDataType  lastCalcDone=0,///temporary solution
        ...
        this->sDDownOut = sDDownOut;      /**  1-bit out   output-port    Sum  **/
        this->sDDownEn  = sDDownEn;       /**  1-bit out   output-port    Sum  **/
        this->sDDEOFSet = sDDEOFSet;      /**  1-bit out   output-port SddEOFSet  **/
        this->eofClr    = eofClr;         /**  1-bit out    markReg    EndOfF     **/
    ...
    PE_CL1(const PE_CL1& cl1){
        ...
        this->sDDownOut     = cl1.sDDownOut;      /**  1-bit out   output-port    Sum  **/
        this->sDDownEn      = cl1.sDDownEn;       /**  1-bit out   output-port    Sum  **/
        this->sDDEOFSet     = cl1.sDDEOFSet;      /**  1-bit out   output-port SddEOFSet  **/
        this->eofClr        = cl1.eofClr;         /**  1-bit out    markReg    EndOfF     **/
    ...
    inline bool OutputConnectedCorrect() const override{
        return  this->ports[PORT_INDEX(CalcDone )].SameTo(this->calcDone )
             ...
             && this->ports[PORT_INDEX(SddownOut)].SameTo(this->sDDownOut)
             && this->ports[PORT_INDEX(SddownEn )].SameTo(this->sDDownEn );
    }
    ...
    inline bool Run() override{
        if (this->transState == TransType::UNRECEIVED)
            if (this->ReceivedForwarded())
                this->transState = TransType::RECEIVED;
        ...
        case TransType::RECEIVED:
            if (this->DDownCanForward()){
                if (this->IsEndOfForward()){
                    if (this->SummationToSend())
...
//                this->calcDone  = innerCalcDone;
        ...
        case TransType::RECEIVED:
            switch(this->nextState){
            case TransType::UNRECEIVED:
                /** received an forwarded data,
                 ** but the up-up cell is going to send more
                 **/
                this->ForwardUpUp();
                this->ForwardMA();
                break;
        ...
        this->lastCalcDone = innerCalcDone;
        return true;
    }
    ...
    inline void PrintInput(){
        ...
        /** the relevant port of feature, weight, and output-mark **/
            ///--------------- buffer ---------------///
        std::cout<<"Obuf      "<<this->ports[PORT_INDEX(Obuf     )].Data()<<std::endl; /**  1-bit in    reg  output-buffer **/
        std::cout<<"Wbuf      "<<this->ports[PORT_INDEX(Wbuf     )].Data()<<std::endl; /**  8-bit in    reg  weight-buffer **/
        std::cout<<"Xbuf      "<<this->ports[PORT_INDEX(Xbuf     )].Data()<<std::endl; /**  8-bit in    reg feature-buffer **/
        /** the part connect to counter **/
        std::cout<<"CountDone "<<this->ports[PORT_INDEX(CountDone)].Data()<<std::endl; /**  1-bit in    Counter **/
        /** the relevant port of partial sum **/
            ///------------   up of up   ------------///
        std::cout<<"FupupIn   "<<this->ports[PORT_INDEX(FupupIn  )].Data()<<std::endl; /**  1-bit in    markReg UpUpFull **/
        std::cout<<"Sin       "<<this->ports[PORT_INDEX(Sin      )].Data()<<std::endl; /** 24-bit in    reg Sum **/
            ///------------ down of down ------------///
        std::cout<<"FddownIn  "<<this->ports[PORT_INDEX(FddownIn )].Data()<<std::endl; /**  1-bit in     input-port FddownIn  **/
            ///------------ partial reg ------------///
        std::cout<<"InnerSreg "<<this->ports[PORT_INDEX(InnerSreg)].Data()<<std::endl; /** 24-bit in    reg ParSum **/
        std::cout<<"**************************"<<std::endl;
        return;
    }
};
...
private:
    ...
    inline void InitializePorts() override{
        ...
        this->ports.push_back(Port( 1,PortDirection::IN ));/** Sen        1-bit in   upup-cell **/
        this->ports.push_back(Port( 1,PortDirection::IN ));/** EOFSet     1-bit in   upup-cell **/
            ///------------ down of down ------------///
        ...
        this->ports.push_back(Port(24,PortDirection::OUT));/** SddownOut 24-bit out ddown-cell **/
        this->ports.push_back(Port( 1,PortDirection::OUT));/** SddownEn   1-bit out ddown-cell **/
        this->ports.push_back(Port( 1,PortDirection::OUT));/** SddEOFSet  1-bit out ddown-cell **/
        assert(this->ports.size()==PortName::Number());
        return;
    }
    inline void OutputConnect() override{
        ...
        const  PE_CL1::PortName cl1SddownOut   (PE_CL1::PortName::_PortName::SddownOut);
        const  PE_CL1::PortName cl1SddownEn    (PE_CL1::PortName::_PortName::SddownEn );
        const  PE_CL1::PortName cl1SddEOFSet   (PE_CL1::PortName::_PortName::SddEOFSet);
        /** registers **/
        ...
        this->cl1.Clone(
            this->ports[PORT_INDEX(SddownOut)],
            cl1SddownOut
        );
        this->cl1.Clone(
            this->ports[PORT_INDEX(SddownEn )],
            cl1SddownEn
        );
        this->cl1.Clone(
            this->ports[PORT_INDEX(SddEOFSet)],
            cl1SddEOFSet
        );
        assert(this->OutputConnected());
        return;
...
public:
    ...
    public:
        ...
        /** the relevant port of partial sum **/
            ...
            Sen       = 21, /** 1-bit in   upup-cell **/
            EOFSet    = 22, /** 1-bit in   upup-cell **/
            ///------------ down of down ------------///
            FddownIn  = 23, /** 1-bit in  ddown-cell **/
            FddownSet = 24, /** 1-bit out ddown-cell **/
            SddownOut = 25, /**24-bit out ddown-cell **/
            SddownEn  = 26, /** 1-bit out ddown-cell **/
            SddEOFSet = 27, /** 1-bit out ddown-cell **/
        /***** REMARK: *****************************************
    ...
    PE():counter(2,ClockEdge::POS_EDGE){
        ...
        this->markRegs.push_back(MarkReg(ClockedObject::ClockEdge::POS_EDGE));/** EndOfF   1-bit **/
    ...
    void InnerConnect() override{
        ...
        const  PE_CL1::PortName cl1EndOfFow  (PE_CL1::PortName::_PortName::EndOfFow  ); /**  1-bit in     markReg EndOfF **/
        const  PE_CL1::PortName cl1EOFClr    (PE_CL1::PortName::_PortName::EOFClr    ); /**  1-bit out    markReg EndOfF **/
...
//        const  PE_CL1::PortName cl1SddownOut (PE_CL1::PortName::_PortName::SddownOut ); /** 24-bit out   output-port SddownOut **/
//        const  PE_CL1::PortName cl1SddownEn  (PE_CL1::PortName::_PortName::SddownEn  ); /**  1-bit out   output-port SddownEn  **/
        ...
        this->markRegs[MARK_REG_INDEX(EndOfF)].Clone(
            markRegSet,
            this->ports[PORT_INDEX(EOFSet)]
        );
        ...
        /** mark register EndOfF **/
        this->markRegs[MARK_REG_INDEX(EndOfF)].ConnectTo(
            markRegClr,
            this->cl1,
            cl1EOFClr
        );
        ...
        this->cl1.ConnectTo(
            cl1EndOfFow,
            this->markRegs[MARK_REG_INDEX(EndOfF)],
            markRegOut
        );
        this->cl1.ConnectTo(
            cl1Sin,
            this->regs[REG_INDEX(Sum)],
            regOut
        );
        this->cl1.ConnectTo(
            cl1CountDone,
    ...
    bool InnerConnectedCorrect() const override{
        ...
        const  PE_CL1::PortName cl1InnerSout (PE_CL1::PortName::_PortName::InnerSout ); /** 24-bit out   reg ParSum **/
        const  PE_CL1::PortName cl1InnerSen  (PE_CL1::PortName::_PortName::InnerSen  ); /**  1-bit out   reg ParSum **/
        const  PE_CL1::PortName cl1Sout      (PE_CL1::PortName::_PortName::SddownOut ); /** 24-bit out   output-port SddownOut **/
        const  PE_CL1::PortName cl1Sen       (PE_CL1::PortName::_PortName::SddownEn  ); /**  1-bit out   output-port SddownEn  **/
        const Counter::PortName countDone   (Counter::PortName::_PortName::done      ); /**  1-bit out  **/
        const Counter::PortName countEn     (Counter::PortName::_PortName::en        ); /**  1-bit in   **/
        const Counter::PortName countRst    (Counter::PortName::_PortName::rst       ); /**  1-bit in   **/
    ...
    inline void Print(){
        ...
        Object::Port port24( 1,Object::PortDirection::IN );/** SddownEn  24-bit out ddown-cell **/
        ...
        port23.ConnectTo(this->ports[PORT_INDEX(SddownOut)]);/** SddownOut 24-bit out ddown-cell **/
        port24.ConnectTo(this->ports[PORT_INDEX(SddownEn )]);/** SddownEn   1-bit out ddown-cell **/
        ...
        std::cout<<"#    Sum  reg state #"<<std::endl;
        this->regs[REG_INDEX(   Sum)].Print();
        ...
        std::cout<<"SddownOut "<<port23 .Data()<<std::endl;/** SddownOut 24-bit out ddown-cell **/
        std::cout<<"SddownEn  "<<port24 .Data()<<std::endl;/** SddownEn   1-bit out ddown-cell **/
        std::cout<<"#########################"<<std::endl;
        return;
    }
};
